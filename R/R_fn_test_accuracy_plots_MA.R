...
R_fn_sROC_plot_MA <- function(  stan_model_file_name,
            ...
            try({
                Se <- stan_mod_samples$summary(c("Se_baseline"), mean, quantiles = ~ quantile(., probs = c(0.025, 0.50, 0.975))) %>% print(n = 100)
                Sp <- stan_mod_samples$summary(c("Sp_baseline"), mean, quantiles = ~ quantile(., probs = c(0.025, 0.50, 0.975))) %>% print(n = 100)
                Fp <- stan_mod_samples$summary(c("Fp_baseline"), mean, quantiles = ~ quantile(., probs = c(0.025, 0.50, 0.975))) %>% print(n = 100)
                ##
                Se_pred <- stan_mod_samples$summary(c("Se_baseline_pred"), mean, quantiles = ~ quantile(., probs = c(0.025, 0.50, 0.975))) %>% print(n = 100)
                Sp_pred <- stan_mod_samples$summary(c("Sp_baseline_pred"), mean, quantiles = ~ quantile(., probs = c(0.025, 0.50, 0.975))) %>% print(n = 100)
                Fp_pred <- stan_mod_samples$summary(c("Fp_baseline_pred"), mean, quantiles = ~ quantile(., probs = c(0.025, 0.50, 0.975))) %>% print(n = 100)
            })
            ##
            n_thr <- nrow(Se)
            # if (!(is.null(df_true))) { 
