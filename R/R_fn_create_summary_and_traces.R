...
#' @keywords internal
create_summary_and_traces <- function(    package,
                                          use_bridgestan,
                                          use_BayesMVP_for_faster_summaries,
                                          ##
                                          # stan_model_sample_output,
                                          # stan_param_names_list,
                                          internal_obj,
                                          MCMC_params,
                                          ##
                                          model_results,
                                          ...
                                          n_nuisance,
                                          ##
                                          compute_main_params, # excludes nuisance params. and log-lik 
                                          compute_transformed_parameters,
                                          compute_generated_quantities,
                                          ##
                                          save_log_lik_trace,
                                          ##
                                          compute_nested_rhat,
  ...
  if (package == "BayesMVP") {
        ...
        stan_fitted_model <- internal_obj$outs_stan_sampling$stan_mod_samples
        n_nuisance <- 0
  }
  ##
  ...
  if (package == "MetaOrdDTA") {
        ...
        ##
        {
          ...
          stan_functions_dir   <- internal_obj$outs_stan_compile$stan_functions_dir
          # ##
          stan_param_names_list <- internal_obj$outs_stan_init$stan_param_names_list
          # ##
          # n_studies <- internal_obj$n_studies
          # n_cat <- internal_obj$n_cat
          # n_thr <- internal_obj$n_thr
          # ##
        ...
        ##
        {
          ...
          stan_model_outputs$diagnostic_summary  <- internal_obj$outs_stan_sampling$stan_mod_samples$diagnostic_summary()
          stan_model_outputs$sampler_diagnostics <- internal_obj$outs_stan_sampling$stan_mod_samples$sampler_diagnostics()
          stan_model_outputs$draws_array         <- internal_obj$outs_stan_sampling$stan_mod_samples$draws()
        }
        ##
        ## Extract Stan traces:
        ##
        ...
        n_params <- (dim(main_trace_array_w_names)[3])
        ##
        ...
        try({ 
          {
            ...
            ##
            stan_times <- internal_obj$outs_stan_sampling$stan_mod_samples$time()
  ...
  if (package == "MetaOrdDTA") {
          try({
                ...
                ##
                adapt_delta <- MCMC_params$adapt_delta
                max_treedepth <- MCMC_params$max_treedepth  ## For MetaOrdDTA only
                metric_shape  <- MCMC_params$metric_shape  ## For MetaOrdDTA only
                ##
                n_superchains <- MCMC_params$n_superchains
          })
          ##
          try({
            ...
            ##
            n_main <- dim(main_trace[[1]])[1]
            print(paste("n_main = ", n_main))
            ##
  ...
  try({  
        # if (is.null(stan_param_names_list)) { 
          if (package == "BayesMVP") {
                    ...
                    grouped_params <- list()
                    ##
                    grouped_params$names_main    <- internal_obj$outs_stan_init$stan_param_names_list$parameters
                    ##
                    grouped_params$names_tp      <- internal_obj$outs_stan_init$stan_param_names_list$transformed_parameters
                    grouped_params$names_inc_tp  <- c(grouped_params$names_main, internal_obj$outs_stan_init$stan_param_names_list$transformed_parameters) ## new
                    grouped_params$names_tp_wo_log_lik <- grouped_params$names_tp[grouped_params$names_tp != "log_lik"]
                    ##
                    grouped_params$names_gq   <- internal_obj$outs_stan_init$stan_param_names_list$generated_quantities
                    grouped_params$names_inc_tp_and_gq <- c(grouped_params$names_inc_tp, internal_obj$outs_stan_init$stan_param_names_list$generated_quantities) ## new
                    ##
                    grouped_params$pars_names <- grouped_params$bs_names_inc_tp_and_gq
                    ##
                    # print(paste(grouped_params$names_main))
  ...
  try({  
        ...
        if (package == "BayesMVP") {
              ...
              names_all <- pars_names
              param_names  <- names_all
        } else { 
              ##
              ...
              exclude_NA <- FALSE
              ##
              stan_param_names_all <-      find_all_stan_param_names(  stan_draws_array, 
                                                                       debugging = FALSE, 
                                                                       exclude_NA = exclude_NA)
              names_all <- (stan_param_names_all)
              length(names_all)
              ##
              param_names <- names_all
              names <- names_all
        ...
        if (use_bridgestan == FALSE) {
                try({ 
                      ##
                      ## ----  Main parameters ("parameters" block):
                      ##
                      names_main <-     filter_param_names_string_batched(   all_param_names_vec = names_all, 
                                                                             param_string = grouped_params$names_main, 
                                                                            condition = "exact_match",
                                                                            debugging = FALSE)
                      names_main <- unname(unlist(names_main)) ; names_main
                      n_main <- (length(names_main))
                      ##
                      # names_all[150:200]
                      grouped_params$names_tp_wo_log_lik
                      
                      ##
                      ## ---- Transformed parameters  (inc. log_lik):
                      ##
                      names_tp <-    filter_param_names_string_batched(  all_param_names_vec = names_all, 
                                                                         param_string = grouped_params$names_tp, 
                                                                         condition = "exact_match",
                                                                         debugging = FALSE)
                      names_tp <- unname(unlist(names_tp)) ; names_tp
                      n_tp <- length(names_tp) ; n_tp
                      ##
                      index_all  <- 1:length(names_all)
                      index_main <- 1:n_main
                      ##
                      index_main_and_tp <- 1:(n_tp + n_main)
                      index_tp <- setdiff(index_all, index_main_and_tp) ; index_tp
                      
                      ##
                      ## ----  Transformed parameters  (WITHOUT log_lik):
                      ##
                      names_tp_wo_log_lik <- filter_param_names_string_batched(    all_param_names_vec = names_all, 
                                                                                   param_string = grouped_params$names_tp_wo_log_lik, 
                                                                                   condition = "exact_match",
                                                                                   debugging = FALSE)
                      ##
                      names_tp_wo_log_lik <- unname(unlist(names_tp_wo_log_lik)) ; names_tp_wo_log_lik
                      n_tp_wo_log_lik     <- length(names_tp_wo_log_lik) ; n_tp_wo_log_lik
                      ##
                      index_log_lik <- grep("^log_lik", names_all, invert = FALSE) ; index_log_lik
                      n_log_lik <- length(index_log_lik) ; n_log_lik
                      ##
                      index_tp_wo_log_lik <- setdiff(index_log_lik, index_tp)
                      n_tp_wo_log_lik     <- length(index_tp_wo_log_lik) ; n_tp_wo_log_lik
                      ##
                      ## ----  Generated quantities:
                      ##
                      names_gq <- filter_param_names_string_batched(     all_param_names_vec = names_all, 
                                                                         param_string = grouped_params$names_gq,
                                                                         condition = "exact_match",
                                                                         debugging = FALSE)
                      names_gq <- unname(unlist(names_gq)) ; names_gq
                      n_gq <- length(names_gq)
                      ##
                      names_inc_tp_and_gq <- c(names_main, names_tp, names_gq)
                      n_params_inc_tp_and_gq <- length(names_inc_tp_and_gq)
                      ##
                      # length(names_inc_tp_and_gq)
                      # names_inc_tp        <- c(names, stan_param_names_list$transformed_parameters) ## new
                      # names_inc_tp_and_gq <- c(names_inc_tp, stan_param_names_list$generated_quantities) ## new
                      #### names_inc_tp        <-  model$param_names(include_tp = TRUE) ## bookmark_2 (previous code)
                      #### names_inc_tp_and_gq <-  model$param_names(include_tp = TRUE, include_gq = TRUE) ## bookmark_2 (previous code)
                      pars_names <- names_inc_tp_and_gq
                      length(pars_names)
                      ...
                      # }
                      ##
                      #  pars_names <- model$param_names(  include_tp = TRUE, include_gq = TRUE)
                      ##
  ...
  #     n_params_inc_tp <- length(names_inc_tp)
  #     n_params_inc_tp_and_gq <- length(names_inc_tp_and_gq)
  #     ##
  #     index <-  1:n_params
  #     index_inc_tp <-  1:n_params_inc_tp
  #     index_inc_tp_and_gq <-  1:n_params_inc_tp_and_gq
  #     ##
  #     ## Mow find names, indexes and N's of tp and gq ONLY:
  #     ##
  #     index_tp <- setdiff(index_inc_tp, index)
  #     names_tp <- pars_names[index_tp]
  ...
  #     # index_tp <- names_tp_wo_log_lik
  #     ##
  #     index_gq <- setdiff(index_inc_tp_and_gq, index_inc_tp)
  #     names_gq <- pars_names[index_gq]
  #     n_gq <- length(names_gq)
  #     ##
  # })
  ...
  try({
      ...
      if (package == "BayesMVP") {
            ...
            try({ 
              print(paste("n_main = ", n_main))
              index_main <- index_wo_nuisance_and_log_lik[1]:(index_wo_nuisance_and_log_lik[1] + n_main - 1)
              # print(head(index_main))
            })
      } else if (package == "MetaOrdDTA") { 
   ...
   if (package == "BayesMVP") {
      if (use_bridgestan == TRUE) {
         ...
         all_param_outs_trace_inc_log_lik <- BayesMVP:::fn_compute_param_constrain_from_trace_parallel( unc_params_trace_input_main = main_trace,
            ...
            # trace_main_as_posterior_array <- posterior::as_draws_array(trace_main)
            # 
            # str(trace_main_as_posterior_array)
            # 
            # stan_standalone_gq_outs <- stan_model_obj$generate_quantities(  fitted_params = trace_main_as_posterior_array,
            #                                                                 data = stan_data_list,
            #                                                                 seed = 123,
   ...
   offset <- 0
   ##
   ## trace_main <- array(dim = c(n_main, n_iter, n_chains))
   ...
   if (package == "BayesMVP") {
         if (use_bridgestan == TRUE) {
                   ...
                   if (compute_main_params == TRUE) {
                     # kk <- 1
                     # all_param_outs_trace_inc_log_lik[[kk]][index_main - offset, 1:n_iter]
                     ## trace_main[1:n_main, 1:n_iter, kk] <- all_param_outs_trace_inc_log_lik[[kk]][index_main - offset, 1:n_iter]
                      for (kk in 1:n_chains) {
                        try({
                           trace_main[1:n_main, 1:n_iter, kk] <-   all_param_outs_trace_inc_log_lik[[kk]][index_main - offset, 1:n_iter]  ##  main_trace_list[[kk]][1:n_main, 1:n_iter]##  all_param_outs_trace[[kk]][index_main - offset, 1:n_iter] ## BOOKMARK
                        }, silent = TRUE)
                      }
                   ...
                   ##
                   try({  
                     rm(all_param_outs_trace_inc_log_lik)
                     gc() ; gc()
                   })
             ...
             #### BOOKMARK - Put code to compute traces for BayesMVP WITHOUT usign bridgestan here
                 ...
                 stan_draws_array_old <- stan_draws_array
                 # message(paste("stan_draws_array_old = "))
                 # str(stan_draws_array_old)
                 # ##
                 stan_draws_array <- format_named_array_for_bayesplot(stan_draws_array_old)
                 ##
                 try({  
                   rm(stan_draws_array_old)
                   gc() ; gc()
                 })
                 ##
                 # message(paste("stan_draws_array = "))
                 # str(stan_draws_array)
                 # ##
                 ## Main params ("parameters" block):
                 ##
                 if (compute_main_params == TRUE) {
                       # print(str(stan_draws_array))
                       outs <- filter_param_draws_string_batched(   named_draws_array = stan_draws_array,
                                                                    param_strings_vec = grouped_params$names_main,
                                                                    condition = "exact_match",
                                                                    exclude_NA = exclude_NA,
                                                                    debugging = FALSE)
                       ##
                       outs$param_draws_filtered_array
                       trace_main_list <- outs$param_draws_filtered_list
                       # message(paste("trace_main_list = ", str(trace_main)))
                       ##
                       trace_main <- outs$param_draws_filtered_array
                       # message(paste("trace_main = ", str(trace_main)))
                       ##
                       names_main <- dimnames(trace_main)$variable
                       # message(paste("names_main = ", head(names_main, 10)))
                       ##
                       n_main <- length(names_main)
                       # message(paste("n_main = ", n_main))
                       ##
                       try({  
                         rm(outs)
                         gc() ; gc()
                 ...
                 if (compute_transformed_parameters == TRUE) {
                       outs <- filter_param_draws_string_batched(  named_draws_array = stan_draws_array,
                                                                                ...
                                                                                debugging = FALSE)
                       ##
                       trace_tp_list <- outs$param_draws_filtered_list
                       ##
                       trace_tp      <- outs$param_draws_filtered_array
                       ##
                       names_tp <- dimnames(trace_main)$variable
                       n_tp <- length(names_tp)
                       ##
                       try({  
                         rm(outs)
                 ...
                 if (compute_transformed_parameters == TRUE) {
                       outs <- filter_param_draws_string_batched(  named_draws_array = stan_draws_array,
                                                                                ...
                                                                                debugging = FALSE)
                       ##
                       trace_tp_wo_log_lik_list <- outs$param_draws_filtered_list
                       ##
                       trace_tp_wo_log_lik      <- outs$param_draws_filtered_array
                       ##
                       names_tp_wo_log_lik <- dimnames(trace_tp_wo_log_lik)$variable
                       n_tp_wo_log_lik <- length(names_tp_wo_log_lik)
                       ##
                       try({  
                         rm(outs)
                         gc() ; gc()
                 ...
                 if (compute_generated_quantities == TRUE) {
                       outs <- filter_param_draws_string_batched(   named_draws_array = stan_draws_array,
                                                                                 ...
                                                                                 debugging = FALSE)
                       ##
                       trace_gq_list <- outs$param_draws_filtered_list
                       ##
                       trace_gq      <- outs$param_draws_filtered_array
                       ##
                       names_gq <- dimnames(trace_gq)$variable
                       n_gq <- length(names_gq)
                       ##
                       try({  
                         rm(outs)
                 ...
                 if (save_log_lik_trace == TRUE) {
                        outs <- filter_param_draws_string_batched(    named_draws_array = stan_draws_array,
                                                                                   param_string = "log_lik",
                                                                                   condition = "exact_match",
                                                                                   exclude_NA = exclude_NA,
                                                                                   debugging = FALSE)
                       ##
                       trace_log_lik_list <- outs$param_draws_filtered_list
                       ##
                       trace_log_lik      <- outs$param_draws_filtered_array
                       ##
                       names_log_lik <- dimnames(trace_log_lik)$variable
                       n_log_lik <- length(names_log_lik)
                       ##
                       try({  
                         rm(outs)
                         gc() ; gc()
   ...
   chain_dim <- 2
   
     {
          ...
          # names_main
          # # str(trace_main)
          # trace_main_3)$variable
          # imnames(trace_main))
          ##
          # print(str(trace_main))
          ##
          names_main <- dimnames(trace_main)$variable
          n_main <- length(names_main)
          ##
          ...
          if (package == "MetaOrdDTA") { 
                ...
                # trace_main
                dimnames(trace_log_lik)
                n_chains <- dim(trace_main_3)[3]
                n_superchains <- MetaOrdDTA:::if_null_then_set_to(n_superchains, n_chains)
                
          } else if (package == "BayesMVP") { 
            
                names_main <- head(names_wo_nuisance_and_log_lik, n_main)
                n_chains <- dim(trace_main_3)[3]
          ...
          if ((package == "BayesMVP") || (use_BayesMVP_for_faster_summaries == TRUE)) {
                  ...
                  Min_ESS_main <-  min(na.rm = TRUE, summary_tibble_main_params$n_eff[1:n_main])
                  Max_rhat_main <- max(na.rm = TRUE, summary_tibble_main_params$Rhat[1:n_main])
                  ##
                  if (compute_nested_rhat == TRUE) {
                    Max_nested_rhat_main <- max(na.rm = TRUE, summary_tibble_main_params$n_Rhat[1:n_main])
                  }
                  ...
                  summary_tibble_main_params <- MetaOrdDTA:::R_fn_compute_cmdstanr_summary_tibble( stan_fitted_model = stan_fitted_model, 
                                                                                         variables_vec =  grouped_params$names_main)
                  ##
                  Min_ESS_main < - round(min(na.rm = TRUE, summary_tibble_main_params$ess_bulk[1:n_main]))
                  Max_rhat_main <- max(na.rm = TRUE, summary_tibble_main_params$rhat[1:n_main])
   ...
   ##
   # names_main <- dimnames(trace_main)$variable
   # n_tp <- length(names_main)
   # message(paste("n_tp = ", n_tp))
   print(paste("n_tp_wo_log_lik = ", n_tp_wo_log_lik))
   ##
   {
     summary_tibble_transformed_parameters <- NULL
     if ((compute_transformed_parameters == TRUE) && (n_tp_wo_log_lik > 0)) {
       ...
       if ((package == "BayesMVP") || (use_BayesMVP_for_faster_summaries == TRUE)) {
                   ...
                   if (package == "MetaOrdDTA") { 
                       n_chains <- dim(trace_tp_wo_log_lik_3)[3]
                       n_superchains <- MetaOrdDTA:::if_null_then_set_to(n_superchains, n_chains)
                   } else if (package == "BayesMVP") { 
                       n_chains <- dim(trace_tp_wo_log_lik_3)[3]
                   }
                   ...
                   summary_tibble_transformed_parameters <- generate_summary_tibble(   n_threads = n_cores,
                                                                                       input_array = trace_tp_wo_log_lik_3,
                                                                                       iter_dim = iter_dim,
                                                                                       chain_dim = chain_dim,
   ...
   ##
   {
         ...
         # dimnames(trace_main_reshaped) <- list(iterations = 1:n_iter, 
         #                                             chains = 1:n_chains, 
         ...
         try({  
           {
               ...
               # ##
               # str(trace_main) ## make sure its "dims = (iter, chains, params)"
               ...
               if (save_trace_tibbles == TRUE) {
                   trace_main_tibble <- dplyr::tibble(posterior::as_draws_df(trace_main))
         ...
         # {
         #     dims_main <- dim(trace_main)[3] ; dims_main
   ...
   # draws_array[1:n_iter, 1:n_chains, 1:n_main] <- trace_main_reshaped # main first 
   # 
   # if (compute_transformed_parameters == TRUE)   {
   #   draws_array[1:n_iter, 1:n_chains, (n_main + 1):(n_main + n_tp)] <- trace_tp_reshaped
   # }
   # if ((compute_generated_quantities == TRUE) &&  (compute_transformed_parameters == TRUE))   {
   #   draws_array[1:n_iter, 1:n_chains, (n_main + n_tp + 1):(n_main + n_tp + n_gq)] <- trace_gq_reshaped
   # }
   # 
   # if ((compute_generated_quantities == TRUE) &&  (compute_transformed_parameters == FALSE))   {
   #   draws_array[1:n_iter, 1:n_chains, (n_main + 1):(n_main + n_gq)] <- trace_gq_reshaped
   # }
...
#                                   parameters = names_total)
     ...
     ##
  {
          ...
          if (package == "BayesMVP") {
                    ...
                    ##
                    stan_HMC_info_list <- R_fn_compute_L_and_epsilon_from_stan_model( Min_ESS = Min_ESS, 
    ...
    ##
    {
      ...
      try({  
          rm(draws_array, 
             trace_main, trace_tp, trace_tp_wo_log_lik, trace_gq, trace_log_lik)
            gc() ; gc()
      })
      ##
      ## ---- List to store traces (as tibbles/DF's):
      ##
      traces_as_tibbles <- list( trace_main_tibble = trace_main_tibble,
    ...
    ##
     ...
     try({ 
        ...
        ##
        HMC_info$HMC_diagnostic_info <- HMC_diagnostic_info
     })
    ##
    ## ---- "summaries" list:
    ##
    ...
    ##
    try(rm(traces_as_arrays))
    try(rm(traces_as_tibbles))
    try(rm(nuisance_trace))
    try({  
      gc() ; gc()
    })
    ##
