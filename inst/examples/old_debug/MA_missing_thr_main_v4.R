...
Model <- "Jones"
##
Model <- "Cerullo_Xu_FIXED_cutpoints"
Model <- "Cerullo_Xu_RANDOM_cutpoints"
##
Model <- "Cerullo_Gat_FIXED_cutpoints"
Model <- "Cerullo_Gat_RANDOM_cutpoints"
...

{
            ...
            if (Model == "Jones") {
                             ...
                             # file <- file.path(getwd(), "inst", "stan_models", "stan_models_MA", "DTA_MA_Jones.stan")
                             # mod <- cmdstanr::cmdstan_model(file) ## , force_recompile = TRUE)

            } else if (Model == "Cerullo_Xu_FIXED_cutpoints") {

                             {
                               model_args_list$ord_model_parameterisation    <- "Xu"
                               model_args_list$random_thresholds             <- TRUE
                               model_args_list$Dirichlet_random_effects_type <- "none"
                             }
                             # ##
                             # file <- file.path(getwd(), "inst", "stan_models", "stan_models_MA", "DTA_MA_Xu_FIXEDthr.stan")
                             # mod <-  cmdstanr::cmdstan_model(file)
              
            } else if (Model == "Cerullo_Xu_RANDOM_cutpoints") {
             
                             {
                               model_args_list$ord_model_parameterisation    <- "Xu"
                               model_args_list$random_thresholds             <- FALSE
                               model_args_list$Dirichlet_random_effects_type <- "SD"
                             }
                             ##
                             if (cutpoint_param == "sigma") {
                                   # file <- file.path(getwd(), "inst", "stan_models", "stan_models_MA", "DTA_MA_Xu_RANDthr_SD.stan")
                                   # mod <-  cmdstanr::cmdstan_model(file)
                             } else if (cutpoint_param == "kappa") {
                                   # file <- file.path(getwd(), "inst", "stan_models", "stan_models_MA", "DTA_MA_Xu_RANDthr_kappa.stan")
                                   # mod <-  cmdstanr::cmdstan_model(file)
                             }
                             
             } else if (Model == "Cerullo_Gat_FIXED_cutpoints") {
                             
                             {
                               model_args_list$ord_model_parameterisation    <- "Gatsonis"
                               model_args_list$random_thresholds             <- TRUE
                               model_args_list$Dirichlet_random_effects_type <- "none"
                             }
                             # ##
                             # file <- file.path(getwd(), "inst", "stan_models", "stan_models_MA", "DTA_MA_Gat_FIXEDthr.stan")
                             # mod <- cmdstanr::cmdstan_model(file)
                           
             } else if (Model == "Cerullo_Gat_RANDOM_cutpoints") {
                             ...
                             # ##
                             # file <- file.path(getwd(), "inst", "stan_models", "stan_models_MA", "DTA_MA_Gat_RANDthr_SD.stan")
                             # cmdstanr::cmdstan_model(file)
                           
             }
  
             # outs <- R_fn_compile_Stan_model_basic(   Model_type = Model_type,
...
## -----------------  Setup data: --------------------------------------------------------------------------------------------
##
data_setup_outs <- R_fn_prep_data(  x_diseased = x_d, 
                                    x_non_diseased = x_nd, 
                                    n_thr = n_thr)
##
model_args_list$Stan_data_list <- data_setup_outs$Stan_data_list ## update "model_args_list"
model_args_list$n_studies <- data_setup_outs$n_studies
model_args_list$n_cat <- data_setup_outs$n_cat
model_args_list$n_thr <- data_setup_outs$n_thr ; model_args_list$n_thr
##
## Get / set basic model info + find the Stan file name:
##
info_outs <- R_fn_get_and_set_basic_model_info(Model_type, model_args_list)
...
##
Stan_model_name_outs <- R_fn_get_Stan_model_file_name(Model_type, model_args_list)
Stan_model_file_name <- Stan_model_name_outs$Stan_model_file_name
model_args_list <- Stan_model_name_outs$model_args_list
##
## ----------------- Compile Stan model: -------------------------------------------------------------------------------------
##
outs_compile <- R_fn_compile_Stan_model_basic( debugging = FALSE, 
                                               Model_type = Model_type,
                                               model_args_list = model_args_list)
model_args_list$stan_model <- outs_compile$mod ## update "model_args_list"
##
## ----------------- Set initial values (using R package defaults): ----------------------------------------------------------
##
model_args_list$n_thr
outs_inits <- R_fn_set_inits(Model_type, model_args_list)
model_args_list$inits <- outs_inits$inits ## update "model_args_list"
##
## -----------------  Set priors (using R package defaults): -----------------------------------------------------------------
##
outs_priors <- R_fn_set_priors_MA(Model_type, model_args_list)
model_args_list$priors <- outs_priors$priors ## update "model_args_list"
model_args_list$priors$prior_boxcox_lambda_SD
model_args_list$priors
##
## -----------------  Run/compile Stan model + save relevant outputs: --------------------------------------------------------
##
## Sample the Stan model:
##
{
    n_chains <- if_null_then_set_to(n_chains, 4)
    ...
    max_treedepth <- if_null_then_set_to(max_treedepth, 10)
    metric_shape  <- if_null_then_set_to(metric_shape, "diag_e") 
}
##
...
##
{
    ...
    init_lists_per_chain <- rep(list(model_args_list$inits), n_chains_burnin) 
    ##
    Stan_data_list <- R_fn_make_Stan_data_list_from_model_args(debugging = FALSE, model_args_list)
    ##
...
try({
      ##
      Stan_model_file_path <-  system.file(package = "MetaOrdDTA", file.path("stan_models", "stan_models_MA", Stan_model_file_name))
      str(Stan_data_list)
      ##
      Stan_data_list <- R_fn_remove_duplicates_from_list(Stan_data_list)
      ##
      outs_init_stan_model <- R_fn_init_Stan_inits_external(  Stan_data_list = Stan_data_list,
                                                              Stan_model_file_path = Stan_model_file_path,
                                                              n_chains_burnin = n_chains_burnin,
                                                              init_lists_per_chain = init_lists_per_chain)
      ##
      # outs_init_stan_model <- R_fn_init_Stan_inits_internal(  Stan_data_list = Stan_data_list,
      #                                                         Stan_model_name = "LC_MVP_bin_PartialLog_v5.stan",
      #                                                         n_chains_burnin = n_chains_burnin,
      #                                                         init_lists_per_chain = init_lists_per_chain)
      ##
      inits_unconstrained_vec_per_chain <- outs_init_stan_model$inits_unconstrained_vec_per_chain ## optional for this pkg
      ##
      param_names_list <- outs_init_stan_model$param_names_list
      param_names_main <- outs_init_stan_model$param_names_main
      ##
      Stan_sampling_outs   <- outs_init_stan_model$Stan_sampling_outs
      stan_model           <- outs_init_stan_model$stan_model
...
##
try({
      Stan_model_samples <- MetaOrdDTA:::R_fn_sample_model(    Model_type = Model_type, 
                                                  model_args_list = model_args_list, 
                                                  n_chains = n_chains, 
                                                  init_lists_per_chain = init_lists_per_chain,
                                                  seed = seed,
      ...
      ##
      {
          Stan_mod_samples <- Stan_model_samples$Stan_mod_samples
          ##
          ## Get all outputs:
          ##
          stan_model <- Stan_model_samples$stan_model
          Stan_mod_samples <- Stan_model_samples$Stan_mod_samples
          Stan_data_list <- Stan_model_samples$Stan_data_list
          model_args_list <- Stan_model_samples$model_args_list
          ##
          seed <- Stan_model_samples$seed
          n_chains <- Stan_model_samples$n_chains
          init_lists_per_chain <- Stan_model_samples$init_lists_per_chain
          n_superchains <- Stan_model_samples$n_superchains
          n_iter <- Stan_model_samples$n_iter
          n_burnin <- Stan_model_samples$n_burnin
          adapt_delta <- Stan_model_samples$adapt_delta
          max_treedepth <- Stan_model_samples$max_treedepth
          metric_shape <- Stan_model_samples$metric_shape
          time_total <- Stan_model_samples$time_total
      }
      ##
})
##
## Make "stan_model_outputs" list:
##
{
    stan_model_outputs <- list()
    stan_model_outputs$diagnostic_summary  <- Stan_mod_samples$diagnostic_summary()
    stan_model_outputs$sampler_diagnostics <- Stan_mod_samples$sampler_diagnostics()
    stan_model_outputs$draws_array         <- Stan_mod_samples$draws()
}
##
## -----------------  Make model summary and traces + other outputs:--------------------------------------------------------
##
# stan_model_outputs
##
Stan_model_samples = Stan_model_samples
stan_model_outputs = stan_model_outputs
##
param_names_list = param_names_list
param_names_main = param_names_main
##
summary_and_traces_outs <- create_summary_and_traces(  Stan_model_samples = Stan_model_samples,
                                                       stan_model_outputs = stan_model_outputs,
                                                       param_names_list = param_names_list,
                                                       param_names_main = param_names_main)
##
{
    summaries <- summary_and_traces_outs$summaries
    traces    <- summary_and_traces_outs$traces
    ##
...
##
plot_param_group_batched( draws_array = model_outputs$draws_array,
                          param_prefix = "beta",
                          plot_type = "density",
                          batch_size = 12)
##
plot_multiple_params_batched( draws_array = model_outputs$draws_array,
                              param_prefixes =  c("beta_mu", "raw_scale_mu"),
                          plot_type = "density",
                          batch_size = 12)
##
...
str(model_outputs$draws_array)

main_trace_array_w_names <- model_outputs$draws_array
str(main_trace_array_w_names)

n_params <- dim(main_trace_array_w_names)[3]

main_trace_list <- list()
for (kk in 1:n_chains) {
  main_trace_list[[kk]] <- array(NA, dim = c(n_iter, n_params))
  main_trace_list[[kk]] <- t(array(main_trace_array_w_names[1:n_iter, kk, ], dim = c(n_iter, n_params)))
}
 
str(main_trace_list)
    
str(main_trace_array_w_names[1:n_iter, kk, ])
...
# 
# model_args_list$priors$prior_boxcox_lambda_mean
# 
# model_args_list$use_box_cox <- 0 
# model_args_list$use_softplus_for_scales <- 0
# 
# model_args_list$Stan_data_list
# 
