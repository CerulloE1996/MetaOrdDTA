...
model {
        ...
        for (c in 1:2) {
                ...
                matrix[n_studies, n_thr] Ind_Dir_cumul_prob = inv_logit(C[c]);
                ...
                ////
                for (s in 1:n_studies) {
                    row_vector[n_thr] rho = Ind_Dir_cumul_prob[s, ] .* (1.0 - Ind_Dir_cumul_prob[s, ]); ////  std_normal_pdf(C[c][s, ]); //// , Ind_Dir_cumul_prob[s, 1:n_thr]);
...
generated quantities {
          ...
          ////
          array[2] vector[n_thr] C_mu; //// = cumul_probs_to_C_logit(prob_cumul_mu, 0.0, 1.0);
          for (c in 1:2) {
            ...
            C_mu[c] = cumul_probs_to_C_logit(prob_cumul_mu, 0.0, 1.0);
          }
          ////
          vector[n_thr] Fp = inv_logit(-(C_mu[1] - beta_mu[1]));
          vector[n_thr] Sp = 1.0 - Fp;
          vector[n_thr] Se = inv_logit(-(C_mu[2] - beta_mu[2]));
          ////
          ...
          for (c in 1:2) {
            ...
            C_pred[c] = cumul_probs_to_C_logit(prob_cumul_pred, 0.0, 1.0);  //// Compute PREDICTED cutpoints.
          }
          ////
          vector[2] beta_pred =  multi_normal_cholesky_rng(beta_mu, beta_L_Sigma);
          ////
          vector[n_thr] Fp_pred = inv_logit(-(C_pred[1] - beta_pred[1]));
          vector[n_thr] Sp_pred = 1.0 - Fp_pred;
          vector[n_thr] Se_pred = inv_logit(-(C_pred[2] - beta_pred[2]));
          ////
