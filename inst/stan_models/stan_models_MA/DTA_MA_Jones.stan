...
transformed parameters {
        ...
        matrix[n_studies, 2] raw_scale; // NOT local - needed for log_get_J later
        {
                ...
                ////
                array[2] matrix[n_studies, n_thr] latent_cumul = map_latent_cumul_prob_to_fixed_C(C, beta, scale, n_studies, n_obs_cutpoints, cutpoint_index);  // local
                ////
...
generated quantities { 
          ...
          ////
          vector[n_thr] Fp = inv_logit(-(C - beta_mu[1])/scale_mu[1]);
          vector[n_thr] Sp = 1.0 - Fp;
          vector[n_thr] Se = inv_logit(-(C - beta_mu[2])/scale_mu[2]);
          ////
          ...
          ////
          vector[n_thr] Fp_pred = inv_logit(-(C - beta_pred[1])/scale_pred[1]);
          vector[n_thr] Sp_pred = 1.0 - Fp_pred;
          vector[n_thr] Se_pred = inv_logit(-(C - beta_pred[2])/scale_pred[2]);
          ////
