...
parameters {
          ...
          matrix[n_studies, 2] raw_scale_eta_z;  //// Standard normal RVs for study-level effects - eta[s, 1:2] ~ multi_normal({0, 0}, Sigma).
          vector<lower=0.0>[2] raw_scale_sigma;               //// Variance component - Between-study SD (Nyaga's σ)
          ////
          ...
          matrix<lower=0.0>[n_index_tests, 2] raw_scale_tau;        //// Variance component - Test-specific SD (Nyaga's τ) - delta_{s, c, t} ~ normal(0, tau_{c, t}).
          array[n_index_tests] matrix[n_studies, 2] raw_scale_delta_z;  //// Standard normal RVs for test-specific effects
...
transformed parameters {
          ...
          ////
          matrix[n_studies, 2] beta_eta;  // first eta corresponds to beta and 2nd eta corresponds to raw_scale.
          matrix[n_studies, 2] raw_scale_eta;  // first eta corresponds to beta and 2nd eta corresponds to raw_scale.
          ////
                ...
                for (s in 1:n_studies) {
                      if (indicator_index_test_in_study[s, t] == 1) {
                              ...
                              for (cut_i in 1:n_obs_cutpoints[s, t]) { //// only loop through the OBSERVED cutpoints for test t in study s
                                    ...
                                    real raw_beta_nd_baseline  = beta_mu[t, 1] + beta_eta[s, 1] + beta_delta[t][s, 1]; //// pi_{s, c, t} = Phi(mu_{c, t} + eta_{s, c} + delta_{s, c, t}).
                                    real raw_beta_d_baseline   = beta_mu[t, 2] + beta_eta[s, 2] + beta_delta[t][s, 2]; //// pi_{s, c, t} = Phi(mu_{c, t} + eta_{s, c} + delta_{s, c, t}).
                                    ...
                                    ////
                                    locations[t][1, s] =  raw_beta_nd_baseline; 
                                    locations[t][2, s] =  raw_beta_d_baseline;  
                                    scales[t][1, s] = ((softplus == 1) ? softplus_scaled_jacobian(raw_scale_nd_baseline) : exp_jacobian(raw_scale_nd_baseline)); 
                                    scales[t][2, s] = ((softplus == 1) ? softplus_scaled_jacobian(raw_scale_d_baseline)  : exp_jacobian(raw_scale_d_baseline)); 
...
model {
          ...
          ////
          for (t in 1:n_index_tests) {
               beta_tau[t, 1:2]        ~ normal(0.0, prior_beta_tau_SD[t]);       //// delta_{s, i, t} ~ normal(0, tau_{i, t}):
               raw_scale_tau[t, 1:2]   ~ normal(0.0, prior_raw_scale_tau_SD[t]);  //// delta_{s, i, t} ~ normal(0, tau_{i, t}):
          }
          ////
...
generated quantities {
          ...
          for (t in 1:n_index_tests) {
            for (k in 1:n_thr[t]) {
                  Fp[t][k] =   1.0 - Phi_approx((C_MU[t][k] - beta_mu[t, 1])/softplus_scaled(raw_scale_mu[t, 1]));
                  Sp[t][k] =   1.0 - Fp[t][k];
                  Se[t][k] =   1.0 - Phi_approx((C_MU[t][k] - beta_mu[t, 2])/softplus_scaled(raw_scale_mu[t, 2]));
            }
          }
          ////
