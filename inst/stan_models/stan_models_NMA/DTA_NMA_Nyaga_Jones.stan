...
transformed parameters {
          ...
          ////
          for (t in 1:n_index_tests) {
            locations[t] = rep_matrix(0.0, n_studies, 2);
            scales[t]    = rep_matrix(0.0, n_studies, 2);
          }
          ////
          for (t in 1:n_index_tests) {
                for (s in 1:n_studies) {
                      if (indicator_index_test_in_study[s, t] == 1) {
                              ...
                              for (c in 1:2) {
                                   locations[t][s, c] = beta_mu[t, c];
                                   scales[t][s, c]    = ((softplus == 1) ? softplus_scaled_jacobian(raw_scale_mu[t, c])  : exp_jacobian(raw_scale_mu[t, c]));
                                   for (k in 1:n_thr[t]) {
                                       latent_cumul_prob[t, c][s, k] = (C[t][k] - locations[t][s, c])/scales[t][s, c];
                                   }
                              }
                      }
          ...
          ////
          for (t in 1:n_index_tests) {
              log_lik[t, ] = compute_log_lik_binomial_fact_NMA(cumul_prob[t, ], x[t,,,], n[t,,,], n_obs_cutpoints[, t], indicator_index_test_in_study[, t]);
          }
