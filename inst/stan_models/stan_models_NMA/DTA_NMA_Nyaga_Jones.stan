...
transformed data {
          ...
          ////
          array[n_tests] int start_idx;
          array[n_tests] int end_idx;
          ////
          //// ---- Calculate indices (using void fn):
          ////
          calculate_indices(n_thr, n_tests, start_idx, end_idx);
...
model {
          ...
          ////
          to_vector(raw_scale_mu)  ~ normal(prior_raw_scale_tau_SD(prior_raw_scale_mu_mean), to_vector(prior_raw_scale_mu_SD));
...
generated quantities {
          ...
          for (t in 1:n_index_tests) {
                  int n_thr_t = n_thr[t];
                  Fp[t][1:n_thr_t] =   1.0 - Phi_approx((C[t][1:n_thr_t] - beta_mu[t, 1])/softplus_scaled(raw_scale_mu[t, 1]));
                  Sp[t][1:n_thr_t] =   1.0 - Fp[t][1:n_thr_t];
                  Se[t][1:n_thr_t] =   1.0 - Phi_approx((C[t][1:n_thr_t] - beta_mu[t, 2])/softplus_scaled(raw_scale_mu[t, 2]));
          }
          ////
          ...
          for (t in 1:n_index_tests) {
                ...
                {
                      int n_thr_t = n_thr[t];
                      Fp_pred[t][1:n_thr_t] =   1.0 - Phi_approx((C[t][1:n_thr_t] - beta_nd_pred)/softplus_scaled(raw_scale_nd_pred));
                      Sp_pred[t][1:n_thr_t] =   1.0 - Fp_pred[t][1:n_thr_t];
                      Se_pred[t][1:n_thr_t] =   1.0 - Phi_approx((C[t][1:n_thr_t] - beta_d_pred)/softplus_scaled(raw_scale_d_pred));
                }
          }
          ////
