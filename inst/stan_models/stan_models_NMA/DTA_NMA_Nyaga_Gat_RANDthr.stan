                ...
                for (s in 1:n_studies) {
                      if (indicator_index_test_in_study[s, t] == 1) {
                                  ...
                                  for (cut_i in 1:n_obs_cutpoints[s, t]) { //// only loop through the OBSERVED cutpoints for test t in study s
                                        ...
                                        scales[t][1, s] = exp(-0.5*raw_scale_baseline); //// D- group //// softplus_scale(-0.5*(raw_scale_mu + raw_scale_SD * raw_scale_z[s]));
                                        scales[t][2, s] = exp(+0.5*raw_scale_baseline); //// D+ group //// softplus_scale(+0.5*(raw_scale_mu + raw_scale_SD * raw_scale_z[s]));
