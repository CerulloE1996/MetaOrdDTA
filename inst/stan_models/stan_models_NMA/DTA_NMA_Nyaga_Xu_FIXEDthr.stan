...
transformed parameters {
          ...
          {
                    array[n_index_tests, 2] matrix[n_studies, n_thr_max] cond_prob  = init_nested_array_of_matrices(n_studies, n_thr_max, n_index_tests, 2, 1.0); // local
                    array[n_index_tests, 2] matrix[n_studies, n_thr_max] latent_cumul = init_nested_array_of_matrices(n_studies, n_thr_max, n_index_tests, 2, positive_infinity());
                    ////
                    array[n_index_tests] matrix[n_studies, 2] locations_beta = init_array_of_matrices(n_studies, 2, n_index_tests, 0.0);
                    ////
                    for (t in 1:n_index_tests) {
                                ...
                                // if (indicator_index_test_in_study[s, t] == 1) {
                                    for (cut_i in 1:n_obs_cutpoints[s, t]) { //// only loop through the OBSERVED cutpoints for test t in study s
                                        ...
                                        if (k < n_thr[t]) {
                                            ////
                                            for (c in 1:2) {
                                                locations_beta[t][s, c] =  beta_mu[t, c] + beta_eta[s, c] + beta_delta[t][s, c];
                                                latent_cumul[t, c][s, cut_i] = (C_vec_t[k] - locations_beta[t][s, c]);
                                            }
                                        }
                                    }
                                // }
                    ...
                    ////
                    for (t in 1:n_index_tests) {
                        array[2, 3] matrix[n_studies, n_thr_max] log_lik_outs;
                        log_lik_outs  = compute_log_lik_binomial_probit_fact_NMA( latent_cumul[t, ], 
                                                                          x[t, ],
                                                                          n[t, ],
                                                                          n_obs_cutpoints[, t], 
