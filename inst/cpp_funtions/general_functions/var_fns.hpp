...
#define EIGEN_DONT_PARALLELIZE
 ...
 Eigen::Matrix<stan::math::var, -1, -1 >    Pinkney_LDL_bounds_opt( int K,
                                                                    Eigen::Matrix<stan::math::var, -1, -1 >  lb,
                                                                    Eigen::Matrix<stan::math::var, -1, -1 >  ub,
                                                                    Eigen::Matrix<stan::math::var, -1, -1 >  Omega_unconstrained_mat,
   ...
   Eigen::Matrix<stan::math::var, -1, 1> col_one_raw = Omega_unconstrained_mat.col(0).segment(1, K - 1); // first col except first element
   Eigen::Matrix<stan::math::var, -1, 1> lb_ub_lp_vec_y_outs = lb_ub_lp_vec_y(col_one_raw, lb.col(0).segment(1, K - 1), ub.col(0).segment(1, K - 1)) ;  // logit bounds
   ...
   for (int i = 2; i < K + 1; ++i) { // i goes from 2 (3rd element) to K (last element + 1). 
     if (known_values_indicator(i-1, 0) == 1) { // i goes from 1 (2nd element i.e. skips 1.0) to K - 1 (i.e., last element). 
       
           L(i-1, 0) = stan::math::to_var(known_values(i-1, 0));
           // Eigen::Matrix<stan::math::var, -1, 1 >  lb_ub_lp_vec_y_out = lb_ub_lp(col_one_raw(i-2), lb(i-1, 0), ub(i-1, 0)) ;  // logit bounds
   ...
   for (int i = 3; i < K + 1; ++i) {
           ...
           for (int j = 2; j < i; ++j) {
                     ...
                     if (known_values_indicator(i-1, j-1) == 1) {
                           ...
                           Eigen::Matrix<stan::math::var, -1, 1 >  lb_ub_lp_outs = lb_ub_lp(Omega_unconstrained_mat(i-1, j-1), low,  up) ;
...
// input vector, outputs upper-triangular 3d array of corrs- double
std::vector<Eigen::Matrix<stan::math::var, -1, -1 > >   fn_convert_std_vec_of_corrs_to_3d_array_var(   std::vector<stan::math::var> input_vec,
                                                                                                       int n_rows,
                                                                                                       int n_arrays) {
...
// input vector, outputs upper-triangular 3d array of corrs- double
std::vector<Eigen::Matrix<stan::math::var, -1, -1 > >   fn_convert_Eigen_vec_of_corrs_to_3d_array_var(   Eigen::Matrix<stan::math::var, -1, 1 >   input_vec,
                                                                                                         int n_rows,
                                                                                                         int n_arrays) {
