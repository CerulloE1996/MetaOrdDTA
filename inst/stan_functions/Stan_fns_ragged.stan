...
vector get_test_t_study_s_segment_values( vector original, 
                                          int t, 
                                          int s,  // This should be study_idx (1 to n_studies_per_test[t])
                                          array[] int n_thr, 
                                          array[] int n_studies_per_test) {  // Changed from n_studies
                                        
        // Calculate the start index for this test-study combination
        int start_idx = 0;
        
        // Add all thresholds from previous tests across all their studies
        for (prev_t in 1:(t-1)) {
            start_idx += n_thr[prev_t] * n_studies_per_test[prev_t];  // Use n_studies_per_test[prev_t]
        }
...
vector update_test_t_study_s_segment_values( vector flat_values_to_update, 
                                             vector new_values, 
                                             int t, 
                                             int s,  // This should be study_idx
                                             array[] int n_thr, 
                                             array[] int n_studies_per_test) {
      ...
      // Add all thresholds from previous tests across all their studies
      for (prev_t in 1:(t-1)) {
          start_idx += n_thr[prev_t] * n_studies_per_test[prev_t];
      }
